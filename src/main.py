...
SKIP_GET_REDDIT = True
RUN_LITELLM = False  # flip to False to skip this step
...
# Scheduling flags
RUN_SCHEDULE = os.environ.get("RUN_SCHEDULE", "0").lower() not in ("0", "false", "no", "")
SCHEDULE_TZ = os.environ.get("SCHEDULE_TZ", "UTC")  # default GMT/UTC
SCHEDULE_HOUR = int(os.environ.get("SCHEDULE_HOUR", "2"))  # 02:00 by default
...
# -----------------------------
# Pipeline (wrapped so we can schedule)
# -----------------------------
def run_pipeline() -> None:
    ...
    print("\n[Reddit] Loading…")
    reddit_df = data_helpers.load_reddit_range(
    ...
    # Fallback to newest local .jsonl
    ...
    # Build meta/features and keep only rows with non-empty 'question'
    ...
    # Tokenize (pass records, not the DataFrame)
    ...
    if RUN_LITELLM:
        try:
            ...
            if PUSH_TO_SQL:
                try:
                    rows = load_trends(litellm_result_path)  # TODO: avoid JSON intermediary
...
# -----------------------------
# Entrypoint: manual vs scheduled
# -----------------------------
if __name__ == "__main__":
    if RUN_SCHEDULE:
        # Light, robust scheduler with explicit timezone support
        try:
            from apscheduler.schedulers.blocking import BlockingScheduler
            from apscheduler.triggers.cron import CronTrigger
        except ImportError as e:
            raise SystemExit(
                "RUN_SCHEDULE=1 requires APScheduler. Install it with:\n"
                "  pip install apscheduler pytz\n"
            )

        # Note: 'UTC' == GMT (no DST). Change SCHEDULE_TZ to 'Europe/London' if you want DST.
        print(
            f"⏰ Scheduling daily run at {SCHEDULE_HOUR:02d}:{SCHEDULE_MINUTE:02d} "
            f"in timezone {SCHEDULE_TZ} (set RUN_SCHEDULE=0 to run once)."
        )
        scheduler = BlockingScheduler(timezone=SCHEDULE_TZ)
        trigger = CronTrigger(hour=SCHEDULE_HOUR, minute=SCHEDULE_MINUTE)
        scheduler.add_job(run_pipeline, trigger=trigger, name="periscope_daily_run")
        try:
            scheduler.start()
